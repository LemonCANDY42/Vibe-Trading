...
def _normalize_llm_usage(usage: Any) -> dict[str, int] | None:
    ...
    cache_creation_input_tokens = _coerce_usage_int(usage.get("cache_creation_input_tokens"))
    cache_read_input_tokens = _coerce_usage_int(usage.get("cache_read_input_tokens"))
    total_tokens = _coerce_usage_int(usage.get("total_tokens"))
    if total_tokens == 0 and (input_tokens or output_tokens or cache_creation_input_tokens or cache_read_input_tokens):
        total_tokens = input_tokens + output_tokens + cache_creation_input_tokens + cache_read_input_tokens

    if not (input_tokens or output_tokens or cache_creation_input_tokens or cache_read_input_tokens or total_tokens):
        return None
...
def _record_llm_usage(
    ...
    for key in ("cache_creation_input_tokens", "cache_read_input_tokens"):
        if key in normalized:
            summary[key] = int(summary.get(key) or 0) + normalized[key]
